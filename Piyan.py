...
def add_reply(interaction, key, reply):
  cant_add_list = load(Path('Data/cant_add/cant_add.json'))
  black_list = load(Path('Data/member_list/black_list.json'))
  ...
  if interaction.user.id in black_list:
    ...
    return '李家豐鎖黑名單'
  dictPath = Path('Data/guild_data', f'{interaction.guild.id}/dictionary.json')
  ...
  dicTxt = Path('Data/guild_data',
                f'{interaction.guild.id}/{interaction.guild.name}.txt')
  dicTxt.parent.mkdir(parents=True, exist_ok=True)

  with open(dicTxt, "w", encoding='utf-8') as f:  # 備份
    f.write(
      str(load(dictPath)).replace('{', '{\n').replace('}', '\n}').replace(
        '], ', '], \n'))
...
async def record_dm(message):
  if isinstance(
      message.channel,
      discord.DMChannel) and message.author.id != 1043082295764078652:
    t = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"{t} | {message.author}: {message.content}")
    DM_path = Path('Chat/DM', f'{message.author.id}.txt')
...
async def record_guild(message):
  guild_path = Path('Chat/Guild', f'{message.guild.name}',
                    f'{message.channel.name}.txt')
...
def responder(m):
  dictPath = Path('Data/guild_data', f'{m.guild.id}/dictionary.json')
  try:
...
def say(interaction, text):
  say_path = Path(f'Data/guild_data/{interaction.guild.id}/say.txt')
...
def VIP(vip_list):
  vip_path = Path(f'Data/member_list/{vip_list}.json')
...
def create_embed():
  ...
  embed.set_author(
    name='Piyan#0765',
    url=
    'https://discord.com/api/oauth2/authorize?client_id=1043082295764078652&permissions=8&scope=bot',
    icon_url='https://imgur.com/46ScKGF.jpg')

  # 增加縮圖
  embed.set_thumbnail(url='https://imgur.com/9f3yb2j.jpg')
  ...
  # 增加圖片
  embed.set_image(url='https://imgur.com/akGEDC0.jpg')

  # 增加尾註
  embed.set_footer(text='--Piyan', icon_url='https://imgur.com/KiT5Wbh.jpg')
...
def amount_search(guildID, author):
  dictPath = Path('Data/guild_data', f'{guildID}/dictionary.json')
  dict = load(dictPath)
  amount = 0
  for key in dict:
    try:
